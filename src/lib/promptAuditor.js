...
function buildAuditPrompt(generatedPrompt, config) {
  ...
  return `Você é um auditor especialista em prompts para assistentes com IA no formato BotConversa (WhatsApp).
...
- description: explique o que está errado e o que vai acontecer de errado no assistente com IA, em linguagem de leigo.
...
2. A saída \`saida_fora_escopo\` é por definição um catch-all aberto: ela cobre QUALQUER coisa que não esteja no objetivo do agente — sem precisar enumerar exemplos. Isso é o design correto e intencional. NÃO critique a condição de saida_fora_escopo como "vaga", "ambígua" ou "mal definida". NÃO crie issue sobre "falta de definição do que é fora do escopo". A ausência de listagem explícita É o comportamento correto — tudo não mencionado é fora do escopo por padrão.
